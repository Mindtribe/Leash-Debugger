...
int cc3200_core_detect(void)
{
    ...
    if(!cc3200_core_state.initialized) {RETURN_ERROR(ERROR_UNKNOWN, "CC3200 not init");}

    if(cc3200_core_read_APreg(0, CC3200_CORE_AP_IDR_ADDR, &cc3200_core_state.AP_IDR, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP read fail");}
    if(cc3200_core_state.AP_IDR != CC3200_CORE_AP_IDR) {RETURN_ERROR(ERROR_UNKNOWN, "ID reg fail");}

    uint32_t csw;
    if(cc3200_core_read_APreg(0, CC3200_CORE_AP_CSW_ADDR, &csw, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP read fail");}
    ...
    if(cc3200_core_write_APreg(0, CC3200_CORE_AP_CSW_ADDR,
            ...
            , 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP write fail");}
    ...
    uint32_t cfg;
    if(cc3200_core_read_APreg(0, CC3200_CORE_AP_CFG_ADDR, &cfg, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP read fail");}

    cc3200_core_state.is_big_endian = (cfg & 1);

    if(!cc3200_core_state.device_enabled) {RETURN_ERROR(ERROR_UNKNOWN, "Deviceen fail");}
...
int cc3200_core_read_APreg(uint8_t ap, uint8_t regaddr, uint32_t* result, uint8_t check_response)
{
    ...
    if(cc3200_jtagdp_selectAPBank(ap, (regaddr>>4) & 0xF) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP Bank sel fail");}
    if(cc3200_jtagdp_APACC_read(regaddr & 0xF, result, check_response) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "APACC read fail");}
...
int cc3200_core_write_APreg(uint8_t ap, uint8_t regaddr, uint32_t value, uint8_t check_response)
{
    ...
    if(cc3200_jtagdp_selectAPBank(ap, (regaddr>>4) & 0xF) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP Bank sel fail");}
    if(cc3200_jtagdp_APACC_write(regaddr & 0xF, value, check_response) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "APACC write fail");}
...
int cc3200_core_pipeline_write_APreg(uint8_t ap, uint8_t regaddr, uint32_t len, uint32_t *values)
{
    ...
    if(cc3200_jtagdp_selectAPBank(ap, (regaddr>>4) & 0xF) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP Bank sel fail");}
    if(cc3200_jtagdp_APACC_pipeline_write(regaddr & 0xF, len, values, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "APACC pipe write fail");}
...
int cc3200_core_pipeline_read_APreg(uint8_t ap, uint8_t regaddr, uint32_t len, uint32_t *dst)
{
    ...
    if(cc3200_jtagdp_selectAPBank(ap, (regaddr>>4) & 0xF) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP Bank sel fail");}
    if(cc3200_jtagdp_APACC_pipeline_read(regaddr & 0xF, len, dst) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "APACC pipe read fail");}
...
{
    if(!cc3200_core_state.initialized || !cc3200_core_state.detected) {RETURN_ERROR(ERROR_UNKNOWN, "CC3200 not init");}

    if(cc3200_core_write_APreg(0,CC3200_CORE_AP_TRANSFERADDR_ADDR,addr & 0xFFFFFFF0, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP write fail");}
    if(cc3200_core_read_APreg(0,CC3200_CORE_AP_BD0_ADDR + (addr&0xC),result, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP read fail");}
...
int cc3200_core_pipeline_write_mem_addr(uint32_t addr, uint32_t len, uint32_t* values)
{
    ...
    while(cur_addr < (addr+len*4)){
        if(cc3200_core_write_APreg(0,CC3200_CORE_AP_TRANSFERADDR_ADDR,cur_addr, 0) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP write fail");}
        if(cc3200_core_pipeline_write_APreg(0,CC3200_CORE_AP_DATARW_ADDR,chunk_words,cur_val) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP pipe write fail");}
...
int cc3200_core_pipeline_read_mem_addr(uint32_t addr, uint32_t len, uint32_t *dst)
{
    ...
    while(cur_addr < (addr+len*4)){
        if(cc3200_core_write_APreg(0,CC3200_CORE_AP_TRANSFERADDR_ADDR,cur_addr, 0) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP write fail");}
        if(cc3200_core_pipeline_read_APreg(0,CC3200_CORE_AP_DATARW_ADDR,chunk_words,cur_dst) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP pipe read fail");}
        cur_addr += chunk_words*4;
        next_kb_boundary += 0x400;
        words_left -= chunk_words;
        cur_dst += chunk_words;
...
int cc3200_core_write_mem_addr(uint32_t addr, uint32_t value)
{
    ...
    if(cc3200_core_write_APreg(0,CC3200_CORE_AP_TRANSFERADDR_ADDR,addr & 0xFFFFFFF0, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP write fail");}
    if(cc3200_core_write_APreg(0,CC3200_CORE_AP_BD0_ADDR + (addr&0xC),value, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP write fail");}
...
int cc3200_core_debug_halt(void)
{
    ...
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR,
            ...
            CC3200_CORE_MEM_DHCSR_C_HALT) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}

    uint32_t temp;
    if(cc3200_core_read_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR,
            &temp) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem read fail");}
    if(!(temp & CC3200_CORE_MEM_DHCSR_S_HALT)) {RETURN_ERROR(temp, "CC3200: Halt fail");}
...
{
    if(!cc3200_core_state.initialized || !cc3200_core_state.detected || !cc3200_core_state.halted) {RETURN_ERROR(ERROR_UNKNOWN, "CC3200 not init");}
    ...
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_AIRCR,
            ...
            (CC3200_CORE_MEM_AIRCR_VECTRESET)) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}

    uint8_t halted = 0;
    while(!halted){
        if(cc3200_core_poll_halted(&halted) == RET_FAILURE){
            RETURN_ERROR(ERROR_UNKNOWN, "Halt poll fail");
        }
    }
...
{
    if(!cc3200_core_state.initialized || !cc3200_core_state.detected) {RETURN_ERROR(ERROR_UNKNOWN, "CC3200 not init");}
    if(!cc3200_core_state.halted) {RETURN_ERROR(ERROR_UNKNOWN, "Not halted fail");}

    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR,
            (CC3200_CORE_KEY_DFSR << CC3200_CORE_MEM_DHCSR_DBGKEY_OFFSET) |
            (CC3200_CORE_MEM_DHCSR_C_DEBUGEN) |
            CC3200_CORE_MEM_DHCSR_C_STEP) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}

    uint32_t temp;
    if(cc3200_core_read_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR,
            &temp) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem read fail");}
    if(!(temp & CC3200_CORE_MEM_DHCSR_S_HALT)) {RETURN_ERROR(temp, "Halt fail");}
...
int cc3200_core_debug_continue(void)
{
    ...
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR,
            (CC3200_CORE_KEY_DFSR << CC3200_CORE_MEM_DHCSR_DBGKEY_OFFSET) |
            (CC3200_CORE_MEM_DHCSR_C_DEBUGEN)) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}
...
int cc3200_core_debug_enable(void)
{
    ...
    if(cc3200_core_read_APreg(0, CC3200_CORE_AP_BASE_ADDR, &cc3200_core_state.debug_base_addr, 1) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "AP read fail");}
    ...
    if(cc3200_core_state.compID != CC3200_CORE_COMPID) {RETURN_ERROR(cc3200_core_state.compID, "compID mismatch.");}
    if(cc3200_core_state.perID != CC3200_CORE_PERID) {RETURN_ERROR(cc3200_core_state.perID, "perID mismatch.");}
    ...
    if(cc3200_core_read_mem_addr(cc3200_core_state.debug_base_addr + CC3200_CORE_BASEOFFSET_MEMTYPE,
            &cc3200_core_state.system_memory_present) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem read fail");}
    ...
    //some checks
    if(!cc3200_core_state.has_rom_table) {RETURN_ERROR(ERROR_UNKNOWN, "ROM table fail");} //cc3200 should have a ROM table.
    ...
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR,
            CC3200_CORE_KEY_DFSR << CC3200_CORE_MEM_DHCSR_DBGKEY_OFFSET) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR,
            (CC3200_CORE_KEY_DFSR << CC3200_CORE_MEM_DHCSR_DBGKEY_OFFSET) |
            CC3200_CORE_MEM_DHCSR_C_DEBUGEN) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}
    ...
    if(!(temp & CC3200_CORE_MEM_DHCSR_C_DEBUGEN)){
        RETURN_ERROR(temp, "Debug enable fail");
    }
...
{
    if(!cc3200_core_state.initialized || !cc3200_core_state.detected) {RETURN_ERROR(ERROR_UNKNOWN, "Not init fail");}
    if(!cc3200_core_state.halted) {RETURN_ERROR(ERROR_UNKNOWN, "Not halt fail");} //must be halted to read a register

    uint32_t regnum = (uint32_t) reg;

    //select the register
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DCRSR,
            regnum & CC3200_CORE_MEM_REGSEL_MASK) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}
    ...
    for(i=0; i<100 && !(dhcsr & CC3200_CORE_MEM_DHCSR_S_REGRDY); i++){
        if(cc3200_core_read_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DHCSR, &dhcsr)
                == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem read fail");}
    }
    if(i>=100) {RETURN_ERROR(ERROR_UNKNOWN, "Reg read fail");}
...
{
    if(!cc3200_core_state.initialized || !cc3200_core_state.detected) {RETURN_ERROR(ERROR_UNKNOWN, "CC3200 not init");}
    if(!cc3200_core_state.halted) {RETURN_ERROR(ERROR_UNKNOWN, "Not halt fail");} //must be halted to access a register

    uint32_t regnum = (uint32_t) reg;

    //prepare the data
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DCRDR, value)
            == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}

    //select the register and initialize the transaction
    if(cc3200_core_write_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DCRSR,
            (regnum & CC3200_CORE_MEM_REGSEL_MASK) | CC3200_CORE_MEM_DCRSR_REGWNR) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem write fail");}
...
{
    if(!cc3200_core_state.initialized || !cc3200_core_state.detected) {RETURN_ERROR(ERROR_UNKNOWN, "CC3200 not init");}
    if(!cc3200_core_state.halted) {RETURN_ERROR(ERROR_UNKNOWN, "Not halt fail");}
...
int cc3200_core_getDFSR(uint32_t *result)
{
    if(cc3200_core_read_mem_addr(cc3200_core_state.scs_addr + CC3200_CORE_MEM_DFSR,
            result) == RET_FAILURE) {RETURN_ERROR(ERROR_UNKNOWN, "Mem read fail");}
    return RET_SUCCESS;
}
