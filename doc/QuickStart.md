...
It is recommended to read the [**User Guide**](UserGuide.md)'s section on Flash programming if you plan to use this feature. Here, an example is given programming a **blinky** program onto flash and booting it. It is recommended, if you are not sure of the state of the target's flash memory, to first format it using TI Uniflash.
